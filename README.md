...
## 📱 Offline / PWA
- Service Worker регистрируется из `src/main.jsx` (лог успех/ошибка).
- При установке предкешируются `/index.html`, сборка JS/CSS и иконки, чтобы App Shell грузился офлайн.
- Навигация офлайн: возвращается кешированный `/index.html`, React Router продолжает работать.
- Статика origin: cache-first; OpenLibrary API: network-first с кешом и офлайн-фолбеком.
- После `npm run build` зайдите онлайн через `npm run preview`, чтобы SW закешировал свежие бандлы.
...
```bash
npm run build
npm run preview   # откройте указанный localhost, чтобы SW закешировал сборку
```
